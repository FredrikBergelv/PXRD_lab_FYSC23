...
for i, (start, stop, amplitude) in enumerate(peaks):
    ...
    try:
        ...
        print(f'Peak {i + 1}: Amplitude = {fit_amplitude:.2f} ± {np.sqrt(pcov[0, 0]):.2f}, '
              f'Mean = {fit_mean:.5f} ± {np.sqrt(pcov[1, 1]):.5f}, '
              f'Sigma = {fit_sigma:.5f} ± {np.sqrt(pcov[2, 2]):.5f}, '
              f'FWHM = {fwhm:.5f} ± {fwhm_uncertainty:.5f}')
